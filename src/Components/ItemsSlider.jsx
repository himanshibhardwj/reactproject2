...
import { FaBagShopping, FaRegHeart, FaStar } from 'react-icons/fa6'
...
const ItemsSlider = (props) => {
  return (
    <div className='p-1 h-full featured_all_content shadow-md bg-white flex flex-col items-center gap-5 relative drop-shadow rounded-2xl relative'>
      ...
      <span className={`wishlist_icn absolute top-5 right-5 w-full cursor-pointer`} >
        <FaRegHeart />
      </span>
      <div className='w-full flex flex-col items-center gap-2 pb-10 px-3'>
        ...
        <p className='font-medium text-[15px] deliver_time 2xl:text-[18px] xl:text-[17px] md:text-[18px] lg:text-[15px]  text-[#666666] flex items-center gap-1'>{props.deliver_time}<span className='text-[15px] text-[#000000] font-bold'>{props.time}</span></p>
        ...
        <button className='absolute cursor-pointer top-5 w-[48px] h-[25px] left-5 text-white font-bold text-[10px] discount flex items-center justify-center bg-[#0070BC]'>{props.discount}%</button>
      </div>
    </div>
  )
}
